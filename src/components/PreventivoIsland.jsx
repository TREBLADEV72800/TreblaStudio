...
function WhatsAppIcon(){ return React.createElement('svg',{viewBox:'0 0 24 24', 'aria-hidden':'true', width:18, height:18, fill:'currentColor'}, React.createElement('path',{d:'M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413Z'}));}
...
function PreventivoPage({ initialServices = [] }) {
  ...
  useEffect(() => { if (typeof window !== 'undefined' && initialServices.length === 0) { const p = new URLSearchParams(window.location.search); const svc = p.get('services') || p.get('service'); if (svc) { const list = svc.split(',').map(s=>s.trim().toLowerCase()).filter(Boolean).map(s=>{ if(['site','sito','siti-web','sito-web','siti'].includes(s)) return 'site'; if(['social','social-media','sm'].includes(s)) return 'social'; if(['design','grafiche','grafica'].includes(s)) return 'design'; return null; }).filter(Boolean); const uniq=[...new Set(list)]; if(uniq.length) setForm(c=>({...c, services: uniq})); } } }, []);
  ...
  return (
    <section className="section quote-section">
      ...
      <div className="section-intro">
        ...
        <p>Rispondi a poche domande per ricevere il tuo preventivo. <a href="/prezzi" style={{ color: 'var(--blue)', fontWeight: 800, textDecoration: 'underline', fontSize: '13px' }}>Vedi prezzi chiari</a></p>
      </div>
      ...
      <form className="quote-form" onSubmit={sendQuote} noValidate>
        ...
        <div className="form-block" style={{ borderBottom: 'none', paddingBottom: '8px' }}>
          <label style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', fontWeight: 400, fontSize: '13px', lineHeight: '1.5', cursor: 'pointer' }}>
            ...
            <span>Ho letto l’<a href="/privacy" style={{ color: 'var(--blue)', textDecoration: 'underline', fontWeight: 700 }}>informativa privacy</a> e acconsento al trattamento dei dati per ricevere il preventivo. *</span>
          </label>
        </div>
        ...
        {submitted && (
          <div style={{ marginTop: '14px', border: '1px solid var(--line)', borderRadius: '8px', padding: '14px', background: '#fff', fontSize: '13px' }}>
            ...
            <p style={{ color: 'var(--muted)', margin: '8px 0 0' }}>Ti interessa anche un evento di inaugurazione con musica dal vivo? <a href="/eventi" style={{ color: 'var(--blue)', fontWeight: 800, textDecoration: 'underline' }}>Vedi Eventi →</a></p>
          </div>
        )}
      </form>
    </section>
